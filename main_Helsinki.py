...
def parse_args():
    ...
    parser.add_argument('--city',type=str,default='helsinki',help='experimental city')
    ...
    parser.add_argument('--gam',type=int,default=90,help='sampling ratio')
...
def Unitrain(model,train_loader,val_loader,args):
    ...
    for epoch in range(args.epoch):        
        ...
        train_acc, train_kappa, _ = Unievaluate(train_loader,device,model,criterion_CE,map_dict,poiIDs,T0)
        val_acc, val_kappa, val_loss = Unievaluate(val_loader,device,model,criterion_CE,map_dict,poiIDs,T0)
        ...
        if val_acc > best_acc:
            ...
            best_epoch = epoch
            torch.save(model.state_dict(),f'./ckpt/{args.city}_bs{args.bs}_gam{args.gam}_lam{args.lam}_epoch{epoch}_acc{best_acc}.pth')
...
def main(args):
    ...
    # AOI_POI_Dataset
    valid_AOI_withPOIs_id = np.loadtxt(f'./data/{args.city}_DT_GraphIndexData.txt',dtype=str)
    valid_AOI_withPOIs_id = valid_AOI_withPOIs_id[1:]
    gt = pd.read_csv(f'./data/{args.city}_label_gt.txt')
    gt_type = gt['label'].values # shanghai chengdu/shanghai
    graphType = pd.read_csv('./data/graph_type_17.txt', header=None).values
    ...
    aoi_poi_dataset = AOI_POI_Sample_Dataset(
        filepath=r'Path of the Helsinki dataset',
        ...
        raw_dir=f'./data/{args.city}_DT_GraphData.txt',
        save_dir=f'./cache',
