...
def parse_args():
    ...
    parser.add_argument('--city',type=str,default='beijing',help='experimental city')
    ...
    parser.add_argument('--gam',type=int,default=95,help='sampling ratio')
...
def main(args):
    ...
    gt = pd.read_csv('./data/beijing_label_gt.txt')
    gt_type = gt['label'].values
    ...
    aoi_poi_dataset = AOI_POI_Sample_Dataset(
        filepath=r'Path of the Beijing center dataset',
    ...
    logging.basicConfig(level=logging.INFO,format='%(levelname)s:%(message)s',filename=f'./log/{args.city}_bs{args.bs}_gam{args.gam}_lam{args.lam}.txt')
