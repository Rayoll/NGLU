...
from utils.dataset import China_AOI_POI_Sample_Dataset
from model.HGModel import NGLU_China
...
def main(args):
    ...
    data = pd.read_csv(rf'./data/{args.city}_aoi17_label.csv', dtype={'id': str})
    data['id'] = data['id'].apply(lambda x: x[2:])
    data = data.drop_duplicates(subset='id')
    data = data.sort_values(by='id')
    valid_aoi_id = np.loadtxt(f'./data/cat17V2_{args.city}_DT_GraphIndexData.txt',dtype=str)
    ...
    aoi_poi_dataset = China_AOI_POI_Sample_Dataset(
        filepath=r'Path of the Multi-CUN dataset',
        ...
        raw_dir=f'./data/cat17V2_China_DT_GraphData.txt',
        save_dir=f'./cache/',
    ...
    model = NGLU_China(in_channels=23,hid_channels=128,out_channels=17).to(device)
