...
def parse_args():
    ...
    parser.add_argument('--city',type=str,default='GSR',help='experimental city')
...
def main(args):
    ...
    # AOI_POI_Dataset
    valid_AOI_withPOIs_id = np.loadtxt(f'./data/{args.city}_zone_DT_GraphIndexData.txt',dtype=str)
    valid_AOI_withPOIs_id = valid_AOI_withPOIs_id[1:]
    gt = pd.read_csv(f'./data/{args.city}_zone_label_gt.txt')
    ...
    aoi_poi_dataset = AOI_POI_Sample_Dataset(
        filepath=r'Path of the GSR dataset',
        ...
        raw_dir=f'./data/{args.city}_zone_DT_GraphData.txt',
        save_dir=f'./cache',
