...
def parse_args():
    ...
    parser.add_argument('--city',type=str,default='aomen',help='experimental city')
    ...
    parser.add_argument('--gam',type=int,default=70,help='sampling ratio')
...
def Unitrain(model,train_loader,val_loader,args):
    ...
    for epoch in range(args.epoch):
        ...
        if epoch == 0:
            ...
            T0 = np.percentile(before_sim_list, 95)
        model.sim_list = []
        with tqdm(total=args.ntrain,desc=f'training {epoch+1}') as pbar:
            for img, graph, aoi_labels, graphid in train_loader:
                ...
                loss = criterion_CE(logits_parcel,aoi_labels.long()) + criterion_SML1(med_logits,label_matrix)
...
def main(args):
    ...
    gt = pd.read_csv('./data/aomen_label_gt.txt')
    ...
    aoi_poi_dataset = AOI_POI_Sample_Dataset(
        filepath=r'Path of the Macao dataset',
